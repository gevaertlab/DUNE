...
class Unet(nn.Module):
    def __init__(
        ...
        *,
        ...
        layer_attns_add_rna_cond = True,   # whether to condition the self-attention blocks with the text embeddings, as described in Appendix D.3.1
        ...
        # rna encoding conditioning (optional)

        self.rna_to_cond = None

        if cond_on_rna:
            assert exists(rna_embed_dim), 'rna_embed_dim must be given to the unet if cond_on_rna is True'
            self.rna_to_cond = nn.Linear(rna_embed_dim, cond_dim)

        # finer control over whether to condition on rna encodings

        self.cond_on_rna = cond_on_rna

        # attention pooling

        self.attn_pool = PerceiverResampler(dim = cond_dim, depth = 2, dim_head = attn_dim_head, heads = attn_heads, num_latents = attn_pool_num_latents) if attn_pool_rna else None

        # for classifier free guidance

        self.max_rna_len = max_rna_len

        self.null_rna_embed = nn.Parameter(torch.randn(1, max_rna_len, cond_dim))
        self.null_rna_hidden = nn.Parameter(torch.randn(1, time_cond_dim))

        # for non-attention based text conditioning at all points in the network where time is also conditioned

        self.to_rna_non_attn_cond = None

        if cond_on_rna:
            self.to_rna_non_attn_cond = nn.Sequential(
    ...
    def cast_model_parameters(
        ...
        *,
        ...
        if lowres_cond == self.lowres_cond and \
            ...
            cond_on_rna == self.cond_on_rna and \
            rna_embed_dim == self._locals['rna_embed_dim'] and \
    ...
    def forward(
        ...
        *,
        ...
        if exists(rna_embeds) and self.cond_on_rna:

            # conditional dropout

            rna_keep_mask = prob_mask_like((batch_size,), 1 - cond_drop_prob, device = device)

            rna_keep_mask_embed = rearrange(rna_keep_mask, 'b -> b 1 1')
            rna_keep_mask_hidden = rearrange(rna_keep_mask, 'b -> b 1')
            
            # calculate rna embeds
            rna_tokens = self.rna_to_cond(rna_embeds)

            rna_tokens = rna_tokens[:, :self.max_rna_len]

            if exists(rna_mask):
                rna_mask = rna_mask[:, :self.max_text_len]
                
            rna_tokens_len = rna_tokens.shape[1]
            remainder = self.max_rna_len - rna_tokens_len

            if remainder > 0:
                rna_tokens = F.pad(rna_tokens, (0, 0, 0, remainder))

            if exists(rna_mask):
                if remainder > 0:
                    rna_mask = F.pad(rna_mask, (0, remainder), value = False)

                rna_mask = rearrange(rna_mask, 'b n -> b n 1')
                rna_keep_mask = rna_mask & rna_keep_mask_embed

            null_rna_embed = self.null_rna_embed.to(rna_tokens.dtype) # for some reason pytorch AMP not working

            rna_tokens = rearrange(rna_tokens, 'b n -> b 1 n')

            rna_tokens = torch.where(
                rna_keep_mask_embed,
                rna_tokens,
                null_rna_embed
            )
            ...
            if exists(self.attn_pool):
                rna_tokens = self.attn_pool(rna_tokens)
        ...
        c = time_tokens if not exists(rna_tokens) else torch.cat((time_tokens, rna_tokens), dim = -2)
