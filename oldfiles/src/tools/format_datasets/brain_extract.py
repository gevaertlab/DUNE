...
DATA = join("data/MR/STANFORD/images")
...
if __name__ == "__main__":
    ...
    for patient in tqdm(patients, colour="blue"):
        ...
        print("\n", patient)
        T1= [f for f in os.listdir(join(DATA, patient)) if f.endswith("nii.gz") if not "mask" in f if "T1Gd.nii"in f][0]
        T2= [f for f in os.listdir(join(DATA, patient)) if f.endswith("nii.gz") if not "mask" in f if "FLAIR.nii"in f][0]

        imgs = [T1, T2]

        for seq in imgs: 
                       
            seq_name = "T1Gd" if "T1" in seq else "FLAIR"
            ...
            output_path = join(DATA, patient, f"{seq_name}-masked.nii.gz")
            ...
            if not isfile(output_path):
                ...
                if seq_name =="FLAIR":
                    T1w = nib.load(join(DATA, patient, f"T1Gd-masked.nii.gz"))
