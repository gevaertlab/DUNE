...
def compare_target(net, device, output_dir, testloader, **kwargs):

    imgs, out, _= next(iter(testloader))
    ...
    inputed = format_img(imgs)
    target = format_img(out)
    ...
    reconstructed = format_img(reconstructed)


    inputed = inputed.cpu()
    target = target.cpu()
    
    concat = torch.cat([inputed, reconstructed, target])
    concat = torch.flip(concat, dims=(2,))
    
    grid = make_grid(concat, nrow=nmod*batch_size)
    output_name = f'{output_dir}/logs/reconstructions.png'
...
def train_loop(model, bet, dataloader, optimizer, device, epoch, train, **config):
    ...
    with torch.set_grad_enabled(train):
        for idx, batch in enumerate(tqdm(dataloader, desc=log, colour=colour)):
            ...
            ssimloss = ssim_func(outp, reconst, data_range=images.max())
...
def main(config):
    ...
    trainLoader, testLoader = import_cds(**config)
    ...
    for epoch in range(num_epochs):
        ...
        compare_target(net, device, model_path, testLoader)
