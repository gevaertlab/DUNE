...
# %% Models
def init_survival_predictor(var, restore_models=False, path=None):

    if restore_models:
        mod = restore_model(var,path)
        hyperparams = {}
        ...
        hyperparams = {
            "estimator__n_estimators": np.arange(250, 500, 50),
            "estimator__min_samples_split": np.arange(2, 10)}

    return mod, hyperparams

def init_classifier(var, restore_models=False, path=None):
    if restore_models:
        mod = restore_model(var,path)
        hyperparams = None
    else:
...
def init_regressor(var, restore_models=False, path=None):
    if restore_models:
        mod = restore_model(var,path)
        hyperparams = None
...
def compute_brier(mod, X_test, y_test,  y_train):   
    ...
    survivals = [t[1] for t in np.concatenate([y_train, y_test])]
    lower, upper = np.nanpercentile(survivals, [10, 90])
