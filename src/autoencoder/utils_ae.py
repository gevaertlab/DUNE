...
def parse_arguments(exp):
    ...
    config_file = join("configs", args.config_file.replace(".cfg","") + ".cfg")
...
def import_datasets(        
    ...
    if exists(model_path + "/exports/trainLoader.pth"):
        ...
        fullLoader = DataLoader(
            totalData, batch_size=1, shuffle=True, drop_last=False, num_workers=num_workers)
        trainLoader = DataLoader(
            trainData, batch_size=batch_size, shuffle=True, drop_last=False, num_workers=num_workers)
        testLoader = DataLoader(
            testData, batch_size=batch_size, shuffle=True, drop_last=False, num_workers=num_workers)
