...
cases = {f: join(DATA_FOLDER, f, "study", "series")
         for f in os.listdir(DATA_FOLDER) if isdir(join(DATA_FOLDER, f))}
...
metadata_df["PatientAge"] = metadata_df["PatientAge"].str.extract(
    '(\d+)').astype(int)
