...
DATASET = "UKBIOBANK"
MODS = {"T1":"T1.nii.gz", "T2":"FLAIR.nii.gz"}
...
def ls_dir(path):
    dirs = [join(path, d) for d in os.listdir(path) if os.path.isdir(os.path.join(path, d))]
    return dirs
...
def copy_files(case, files):
    ...
    new_t1 = t1.replace(INPUT_DIR, OUTPUT_DIR).replace(basename(t1), "_init_"+MODS["T1"])
    new_t2 = t2.replace(INPUT_DIR, OUTPUT_DIR).replace(basename(t2), "_init_"+MODS["T2"])
...
def preprocess(case):
    ...
    files = {k:glob(case + f"/*{v}")[0] for k, v in MODS.items()}
