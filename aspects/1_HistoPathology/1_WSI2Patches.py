...
###################################################
# Example command
# $ 1_WSI2Patches.py --wsi_path "/path/to/WSIfolder/" --patch_path "/path/to/output/patchfolder/" --mask_path "/path/to/output/maskfolder/"
...
def extract_patches(slide_path, mask_path, patch_size, patches_output_dir, slide_id, max_patches_per_slide=2000):
    ...
    try:
        with open(os.path.join(patch_folder, 'loc.txt'), 'w') as loc:
            ...
            ratio_x = slide.level_dimensions[PATCH_LEVEL][0] / \
                slide.level_dimensions[mask_level][0]
            ratio_y = slide.level_dimensions[PATCH_LEVEL][1] / \
            ...
            resize_factor = float(
                slide.properties.get('aperio.AppMag', 20)) / 20.0
            resize_factor = resize_factor * args.dezoom_factor
            patch_size_resized = (
                int(resize_factor * patch_size[0]), int(resize_factor * patch_size[1]))
            i = 0
            ...
            for x, y in indices:
                ...
                if mask[x_mask, y_mask] == 1:
                    patch = slide.read_region(
                        (x, y), PATCH_LEVEL, patch_size_resized).convert('RGB')
                    try:
                        mask_patch = get_mask_image(np.array(patch))
                        mask_patch = binary_dilation(
                            mask_patch, iterations=3)
                    except Exception as e:
                        print("error with slide id {} patch {}".format(
                            slide_id, i))
                        print(e)
                    if (mask_patch.sum() > BACKGROUND_THRESHOLD * mask_patch.size) and not (is_low_contrast(patch)):
                        ...
                        imsave(os.path.join(patch_folder, "{0}_patch_{1}.png".format(
                            slide_id, i)), np.array(patch))
                        i += 1
...
parser = argparse.ArgumentParser(
    description='Generate patches from a given folder of images')
