...
def evaluate(model, val_dataloader, criterion, summary_writer, device, epoch, mode='val', task='classification', target_label='label'):
    ...
    if task == 'classification':
        ...
        wsi_acc, wsi_f1, wsi_auc, pandas_output = get_classification_scores(output_list, wsi_list,
                                                                            labels_list)
        case_acc, case_f1, case_auc, _ = get_classification_scores(
            output_list, case_list, labels_list)

        print("{} wsi  | epoch {} | acc {:.3f} | f1 {:.3f} | auc {:.3f}".format(
            mode, epoch, wsi_acc, wsi_f1, wsi_auc))
        print("{} case  | epoch {} | acc {:.3f} | f1 {:.3f} | auc {:.3f}".format(mode, epoch, case_acc, case_f1,
                                                                                 case_auc))

    elif task == 'survival_prediction':
        wsi_CI, pandas_output = get_survival_CI(
            output_list, wsi_list, survival_months_list, vital_status_list)
        case_CI, _ = get_survival_CI(
...
def get_classification_scores_old(output_list, ids_list, labels_list):
    ...
    pandas_output = pd.DataFrame(
        {'id': ids_unique, 'score': score_list, 'label': label_list})
...
def train_model(model, dataloaders, criterion, optimizer, device, save_dir='checkpoints/models/',
    ...
    for epoch in range(num_epochs):
        ...
        for b_idx, batch_dict in enumerate(dataloaders['train']):
            ...
            if (summary_step % log_interval == 0):
                ...
                acc_to_log = (running_corrects -
                              last_running_corrects) / (inputs_seen)
                speed_to_log = log_interval * \
                    input_size[0] / (time.time() - last_time)

                last_time = time.time()
                if summary_writer is not None:
                    summary_writer.add_scalar(
                        "train/loss", loss_to_log, summary_step)
                    summary_writer.add_scalar(
                        "train/acc", acc_to_log, summary_step)
        ...
        train_loss, _ = evaluate(model, dataloaders['train'], criterion, summary_writer,
                                 device, epoch, mode='train', task=task, target_label=target_label)
        val_loss, _ = evaluate(model, dataloaders['val'], criterion, summary_writer,
                               device, epoch, mode='val', task=task, target_label=target_label)

        # print(
        #     f"====>      VAL LOSS is {val_loss} vs BEST VAL LOSS is {best_val_loss} <===============")
    ...
    test_loss, val_output_last = evaluate(model, dataloaders['val'], criterion, summary_writer, device, epoch, task=task, mode='val', target_label=target_label)

    print("\n")
    print("LOADING BEST MODEL, best epoch = {}".format(best_epoch))
    model.load_state_dict(torch.load(
        os.path.join(save_dir, 'model_dict_best.pt')))


    print("EVALUATING ON VAL SET")
    test_loss, val_output_best = evaluate(model, dataloaders['val'], criterion, summary_writer, device, best_epoch, task=task, mode='val', target_label=target_label)

    print("EVALUATING ON TEST SET")
    test_loss, test_output_best = evaluate(model, dataloaders['test'], criterion, summary_writer, device, best_epoch, task=task, mode='test', target_label=target_label)

    if output_dir is not None:
        if not os.path.isdir(output_dir):
            os.makedirs(output_dir)
        val_output_last.to_csv(os.path.join(
            output_dir, 'val_output_last.csv'), index=False)

        val_output_best.to_csv(os.path.join(
            output_dir, 'val_output_best.csv'), index=False)
        test_output_best.to_csv(os.path.join(
            output_dir, 'test_output_best.csv'), index=False)
...
def main():
    ...
    if args.flag == "":
        args.flag = 'train_{date:%Y-%m-%d %H:%M:%S}'.format(
    ...
    model = AggregationModel(resnet=resnet, aggregator=aggregator,
                             aggregator_dim=config['aggregator_hdim'], resnet_dim=2048, out_features=config['num_classes'])
    print("Loaded model")
    ...
    dataloaders_dict = {
        x: torch.utils.data.DataLoader(
            image_datasets[x], batch_size=config['batch_size'], sampler=image_samplers[x], num_workers=config["num_workers"]) for x in ['train', 'val', 'test']
            }
    ...
    layers_to_train = [model.fc, model.resnet.layer4, model.resnet.layer3,
                       model.resnet.layer2, model.resnet.layer1, model.resnet.conv1]
    ...
    train_model(model=model, dataloaders=dataloaders_dict, criterion=criterion,
                ...
                save_dir=os.path.join(
                    args.checkpoint_path, 'models', args.flag),
                task=config.get('task', 'classification'),
                output_dir=os.path.join(
                    args.checkpoint_path, 'outputs', args.flag),
...
parser.add_argument("--save_images", type=int, default=0,
                    help='save sample images from the dataset')
parser.add_argument("--quick", type=int, default=0,
                    help='use small datasets to check that the script runs')
parser.add_argument("--log", type=int, default=0,
                    help='0 = do not use a summary writer')
