...
class brainImages(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        imgsPre = torch.tensor(np.array(imgsPre))
...
def testImageReconstruction(net, testloader, device, Z_slice, ep):
    net.eval()
    ...
    for idx, (imgs, caseid) in enumerate(testloader):
        ...
        save_image(torchvision.utils.make_grid(
            reconstructed, nrow=n_mod), f'{output_dir}_{ep}_reconstr.png')
        save_image(torchvision.utils.make_grid(
            concat, nrow=n_mod), f'{output_dir}_concat.png'
        )
        break
...
def main(train_prop=0.9):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for idx, (images, cases_id) in enumerate(trainLoader):
            ...
            ae_batch_loss = aeCriterion(outputs, images)
        ...
        testImageReconstruction(ae, testLoader, device,
                                Z_slice=MINDIMS[0]//2, ep=epoch)
        plot_losses(train_loss_tracker, test_loss_tracker)

        # Report generation
        model_name = f"V7_{RUN}"
...
if __name__ == "__main__":
    ...
    RUN = f"Batches{BATCH_SIZE}_Blocks{NUM_BLOCKS}_Nodes{NODES}"
