...
class convAE(nn.Module):
    def __init__(self, **kwargs):
        ...
        for i in range(0, self.conv_layers):
            self.ae.append(nn.Conv3d(
                in_channels=kwargs["conv_channels"][i], out_channels=kwargs["conv_channels"][i+1], kernel_size=kwargs["conv_kernel_sizes"]))

        for i in range(0, self.conv_layers):
            self.ae.append(nn.ConvTranspose3d(in_channels=kwargs["conv_channels_backwards"][i],
                           out_channels=kwargs["conv_channels_backwards"][i+1], kernel_size=kwargs["conv_kernel_sizes_backwards"]))
...
class brainImages(Dataset):
    def __init__(self, minDims, labelFile, pathsToData=DATA, modalities=MODALITIES, layersToInclude=5, transforms=None):
        self.folders = []
        for i in range(0, len(pathsToData)):
            #print("the "+str(i)+" path to data = "+str(pathsToData[i]))
            self.folders.append(glob.glob(pathsToData[i]+"/*")[:75])
    ...
    def __getitem__(self, idx):
        ...
        imgsPil = torch.zeros(
            [self.modalityCount, self.minimumDims[1], self.minimumDims[0], self.minimumDims[2]])
        for i in range(0, self.modalityCount):
...
def testImageReconstruction(net, testloader, device, minDims, fileName, slicePad):
    ...
    for batch in testloader:
        ...
        img = img[:, :, :, :, int(minDims[2]/2) -
                  slicePad:int(minDims[2]/2)+slicePad+1]
        img = img.to(device)
        #img = img.view(img.size(0), -1)
        outputs = net(img)
        # outputs.view(outputs.size(0), outputs.size(1), minDims[1], minDims[0], minDims[2]).cpu().data
        outputs = outputs.cpu().data
        outputsT1 = torch.reshape(outputs[:, 0, :, :, slicePad], [
                                  outputs.size(0), 1, minDims[1], minDims[0]])
...
# %%
def fullTrain(testImageFileName=OUTPUT+"check_learning_worked.png", bulkGradients=False, fullVolEval=False, dataSet=None, labels=OUTPUT+'ukb_Sex_BirthYear.csv', numEpochs=3, learningRate=1e-4, trainFrac=0.9, runFrac=1, batchSize=5, slicePad=4, codeDimension=5000, convLayerCount=4, minImageDims=[195, 160, 150], convChannels=[len(MODALITIES), 1, 1, 1, 1], convKernelSizes=[3, 3, 3]):
    ...
    preCodeDimension = computeOuterLinDim(batchSize, slicePad, convLayerCount, minImageDims[0:len(
        minImageDims)-1], convChannels, convKernelSizes)
    ...
    if dataSet == None:
        totalData = brainImages(layersToInclude=2*slicePad+1, minDims=minImageDims, labelFile=labels,
    ...
    for epoch in range(numEpochs):
        ...
        for batch_features, case in trainLoader:
            ...
            for i in range(0, minImageDims[2]-2*slicePad-1):
                ...
                temp_batch = batch_features[:, :, :, :,
                                            shuffleOrder[i]-slicePad:shuffleOrder[i]+slicePad+1]
                ...
                if bulkGradients == False:
                    if fullVolEval == False:
                        ae_train_loss = aeCriterion(
                            aeOutputs[:, :, :, :, slicePad], temp_batch[:, :, :, :, slicePad])
                        ...
                        ae_train_loss = aeCriterion(
                            aeOutputs[:, :, :, :, :], temp_batch[:, :, :, :, :])/(2*slicePad + 1)
                        ae_train_loss.backward()
                        aeOpt.step()
                        aeLoss += ae_train_loss.item()
                        if torch.isnan(ae_train_loss).any():
                    ...
                    temp_loss = aeCriterion(
                        aeOutputs[:, :, :, :, slicePad], temp_batch[:, :, :, :, slicePad])/minImageDims[2]
        ...
        testImageReconstruction(ae, testLoader, device, minImageDims,
                                OUTPUT+"training_test_epoch_"+str(epoch)+".png", slicePad)
    ...
    torch.save(ae.state_dict(), OUTPUT+"brain_ae_model_"+str(preCodeDimension)+"_codeDim_" +
               str(runFrac)+"_runFrac_"+str(fullVolEval)+"_fullVolEval.pth")
    ...
    testImageReconstruction(ae, testLoader, device,
                            minImageDims, testImageFileName, slicePad)
