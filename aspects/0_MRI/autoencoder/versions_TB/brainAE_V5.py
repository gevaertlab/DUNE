...
NUM_CASES = config['n'] if not args.quick else 8*BATCH_SIZE
...
class convAE(nn.Module):
    ...
    def get_code_shape(self, features):
        code, _ = self.encoder(features)
        num_features = np.prod(code.shape)
...
def generate_report(model_name, epoch, train_loss, test_loss, num_features):
    ...
    epoch_summary['quick_exec'] = str(args.quick)
...
def main(train_prop=0.9):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Report generation
        model_name = f"testBatch{BATCH_SIZE}_Blocks{NUM_BLOCKS}_Nodes{NODES}"
...
if __name__ == "__main__":
    ...
    print(f"Quick execution = {args.quick}")
    main()
